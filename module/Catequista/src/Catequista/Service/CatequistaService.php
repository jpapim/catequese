...
class CatequistaService extends Entity
{
    ...
    public function fetchPaginator($pagina = 1, $itensPagina = 5, $ordem = 'id_catequista ASC', $like = null, $itensPaginacao = 5)
    {
        ...
        if (isset($like)) {
            $select
                ...
                ->or
                ->like('id_telefone_residencial', "%{$like}%")
                ->or
                ->like('id_telefone_celular', "%{$like}%")
                ->or
                ->like('em_email', "%{$like}%");
    ...
    public function getCatequistaPaginator($filter = NULL, $camposFilter = NULL)
    {
        ...
        $select = $sql->select('catequista')->columns([
            ...
            ->join('telefone','telefone.id_telefone = catequista.id_telefone_residencial',['nr_ddd_telefone','nr_telefone'])
            ->join('email', 'email.id_email = catequista.id_email');
        $where = [
        ];
        if (!empty($filter)) {
